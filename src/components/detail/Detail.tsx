...
export interface DetailProps {
  ...
  /** What was actually done. Descriptions of work, not claimed outcomes. */
  did?: string[];
  stack: string[];
  ...
  return (
    <>
        ...
        <div className="shell dt__inner">
          ...
          {props.links && props.links.length > 0 && (
            <div className="dt__links" data-reveal>
              {props.links.map((l) =>
                l.kind === "demo" ? (
                  <Link key={l.href} className="btn" href={l.href} data-kind={l.kind}>
                    <span>{l.label}</span>
          ...
          {props.did && props.did.length > 0 && (
            <ul className="dt__did" data-reveal>
              {props.did.map((d) => (
                <li key={d}>{d}</li>
              ))}
            </ul>
      ...
      <style jsx>{`
        ...
        .dt__links :global(a[data-kind="demo"]) {
