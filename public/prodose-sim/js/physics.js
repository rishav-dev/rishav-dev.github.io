(function (G) {
  ...
  function makePill(scene, x, y, angle, id) {
    ...
    body.label = 'pill';
    body.plugin.pill = { id, stress: 0, level: 0, zone: 'bottle', held: false, counted: false, pinchSteps: 0, impacts: 0, maxImpact: 0, fell: false };
    return body;
  }

  // Helpers for moving machine parts.
  class Rig {
  ...
  const SLICK = { wheel: 1, housing: 1, gate: 1, shutter: 1, cap: 1, ramp: 1, chute: 1 }; // polished / PTFE-coated metal parts
  ...
  // Shared simulation state and physics stepping.
  class Scene {
    constructor(params) {
      this.params = params;
      this.design = params.design === 'arm' ? 'arm' : 'wheel';
      ...
      const sealed = this.design === 'wheel' ? params.cap !== false : true;
      this.geo = P.bottleGeometry(params.bottle, this.design === 'wheel' && sealed);
      this.sealedCap = this.design === 'wheel' ? sealed : true;
      this.bottle = buildBottleRig(this.geo, this.design === 'wheel' && sealed);
      Composite.add(this.world, this.bottle.body);

      this.machine = this.design === 'arm' ? new ArmDesign(this) : new WheelDesign(this);
      this.bottlePose(0);
      this.spawnPills();
      this.installEvents();
      this.settle(1.8);
    ...
    startDock() {
      ...
      this.status = this.design === 'wheel' ? (this.sealedCap ? 'Flipping bottle (sealed adapter cap on)' : 'Flipping bottle — NO adapter cap') : 'Raising bottle into sealed dock';
      this.log(this.design === 'wheel'
    ...
    analyze(dt) {
      ...
      for (let i = 0; i < list.length; i++) {
        ...
        if (a.plugin.pill && !a.isStatic && b.isStatic) { pill = a; wall = b; }
        else if (b.plugin.pill && !b.isStatic && a.isStatic) { pill = b; wall = a; }
        else continue;
        ...
        info._c.push({ d: pair.collision.depth, nx, ny, label: wall.label });
      }
      const mat = P.SHAPES[this.spec.shape];
      let worst = 0, worstWheel = false;
      for (const pill of touched) {
        const info = pill.plugin.pill, cs = info._c; info._c = null;
        let best = 0, wheelInvolved = false;
        for (let i = 0; i < cs.length; i++) for (let j = i + 1; j < cs.length; j++) {
          if (cs[i].nx * cs[j].nx + cs[i].ny * cs[j].ny < -0.7) {
            const sum = cs[i].d + cs[j].d;
            if (sum > best) { best = sum; wheelInvolved = cs[i].label === 'wheel' || cs[j].label === 'wheel' || cs[i].label === 'tool' || cs[j].label === 'tool'; }
          }
        }
        if (best > this.pinchDepth) {
          info.pinchSteps++;
          if (best > worst) { worst = best; worstWheel = wheelInvolved; }
          info.stress += ((best - 0.35) * dt) / (260 * mat.fragile);
  ...
  class WheelDesign {
    constructor(scene) {
      ...
      this.wheel.place(0, 0, this.phi.pos);
    }
    ...
    buildWheel() {
      ...
      for (let k = 0; k < this.N; k++) {
        ...
        const fc = at(rf - 3, 0);
        parts.push(rectPart(fc.x, fc.y, pk.w + 2 * tw, 6, pk.a + Math.PI / 2, 'wheel'));
        for (const sg of [-1, 1]) {
        ...
        if (a1 > a0) {
          ...
          for (let i = 0; i < n; i++) {
            ...
            parts.push(rectPart(c.x, c.y, chord + 0.8, 5, tm + Math.PI / 2, 'wheel'));
          }
        }
      }
      const hub = Bodies.circle(0, 0, R - 24, { isStatic: true }); hub.label = 'wheel';
      parts.push(hub);
      this.wheel = new Rig(parts, { label: 'wheel', friction: 0.1, restitution: 0.06 });
      this.wheel.body.frictionStatic = 0.16;
      this.wheel.place(0, 0, 0);
      Composite.add(this.scene.world, this.wheel.body);
    }
    ...
    control(dt) {
      ...
      switch (this.state) {
        ...
        case 'unjam': {
          ...
          if (this.phi.arrived(this._ujTarget)) { this._ujTarget = null; this._tgt = null; this.state = 'agitate'; this.tState = 0; }
          break;
        }
        case 'agitate': {
      ...
      this.wheel.place(0, 0, this.phi.pos + jig);
      if ((this.state === 'step' || this.state === 'shake') && s.pinchWheel && Math.abs(this.phi.vel) > 4 * DEG) this.onJam();
      s.pinchWheel = false;
    ...
    onJam() {
      ...
      this.phi.vel = 0; this.gateOpen = false;
  ...
  class ArmDesign {
    ...
    hold(p) {
      ...
      p.collisionFilter.category = CAT.HELD; p.collisionFilter.mask = 0;
      this.held = p;
    ...
    release() {
      ...
      p.collisionFilter.category = CAT.PILL; p.collisionFilter.mask = CAT.WALL | CAT.PILL | CAT.TOOL;
      Body.setVelocity(p, { x: 0, y: 0.6 }); Body.setAngularVelocity(p, 0);
    ...
    poseHeld(dt) {
      const p = this.held; if (!p) return;
      const t = this.tipWorld, sa = this.shaftAngle;
      const dx = Math.sin(this.alpha.pos), dy = Math.cos(this.alpha.pos);
      const k = Math.min(1, dt / 350);
      this.rel += (this.relTarget - this.rel) * (this.rotGrip ? k : 0);
      const ang = sa + this.rel;
      Body.setAngle(p, ang, true);
      const v = p.vertices, pc = p.position; let m = 0;
      for (let i = 0; i < v.length; i++) { const d = (v[i].x - pc.x) * -dx + (v[i].y - pc.y) * -dy; if (d > m) m = d; }
      Body.setPosition(p, { x: t.x + dx * m, y: t.y + dy * m }, true);
    }
    ...
    control(dt) {
      ...
      switch (this.state) {
        ...
        case 'seal': {
          if (this.tState > 220) {
            const p = this.target.p, tw = this.tipWorld;
            const dx = Math.sin(this.alpha.pos), dy = Math.cos(this.alpha.pos);
            const tilt = this.surfaceTilt(p, dx, dy);
            if (tilt > 48 * DEG || this.cupR * 2 > this.scene.spec.W * 1.05) { this.gripFail(`vacuum seal failed (surface tilted ${(tilt / DEG).toFixed(0)}°)`); }
            else {
              this.hold(p); this.rotGrip = 0; this.state = 'lift'; this.tState = 0;
              s.stats.attempts++; s.log(`Vacuum on: pill #${p.plugin.pill.id} gripped.`, 'info');
            }
          }
          break;
        }
