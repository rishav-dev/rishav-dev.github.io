...
(function () {
  ...
  $('altCards').innerHTML = designs.map((d) => { const a = ALT[d.key], m = A[d.key]; return `<div class="card"><div class="meta"><span class="chip"><i style="background:${d.color}"></i>${label(d)}</span> ${a[0]}</div><h3>${d.name}</h3><p>${a[1]}</p><p><b>Strength.</b> ${a[2]}</p><p><b>Weakness.</b> ${a[3]}</p><p style="color:var(--ink-2)">Bench: dose exact ${pct(m.doseRate.p)}, single-pill ${pct(m.single.p)}, ${m.jamsPerDose.toFixed(2)} jams / dose.</p><p><a href="/work/prodose/simulation/simulator/?design=${d.key}">Open in the simulator →</a></p></div>`; }).join('');
  ...
  const FM = {
    wheel: [['Two pills in one slot', 'Slot wider than one pill; scraper gap too large', (d) => `${pct(A[d.key].multiRate.p, 1)} of cycles`, 'Exit curtain / load cell', 'Slot-size setting + auto scraper gap'], ['Pill pinched at the scraper', 'Pill straddles slot edge under the scraper', (d) => `${A[d.key].jamsPerDose.toFixed(2)} jams / dose`, 'Motor torque hits its limit → stall signal', 'Torque limit at 25 % of crush; reverse and shake'], ['Empty slot', 'Pill did not seat before the slot moved on', (d) => `${pct(A[d.key].missRate.p, 1)} of cycles`, 'Exit barrier sees nothing', 'Vibration assist, dwell, retry']],
    arm: [['Pick fails (no seal)', 'Pill face tilted or too curved for the cup; cup larger than the pill', (d) => `${pct(A[d.key].missRate.p, 1)} of cycles`, 'Vacuum pressure does not drop on contact', 'Pick another pill; cup sized to the pill'], ['Pill dropped in transit', 'Snag on the neck or neighbours needs more force than ΔP·A', (d) => `${pct(A[d.key].missRate.p, 1)} of cycles`, 'Vacuum signal collapses mid-lift', 'Higher vacuum; slower lift; wrist roll for long pills'], ['No reachable pill', 'Last pills in the shoulder corners', (d) => `${A[d.key].errors} of ${A[d.key].n} doses stopped`, 'Vision reports none reachable', 'Tilt the bottle at the end of life']],
    shuttle: [['Double under the wiper', 'Pocket deeper than about one pill; lip yields too far', (d) => `${pct(A[d.key].multiRate.p, 1)} of cycles`, 'Exit curtain', 'Auto pocket depth; lip yield capped below a stacked pill’s protrusion'], ['Pill pinched by the lip', 'Second pill wedged under the wiper', (d) => `${A[d.key].jamsPerDose.toFixed(2)} jams / dose`, 'Servo current limit', 'Compliant lip; back-off and reseat'], ['Pocket empty', 'Fill time too short for the pile to seat a pill', (d) => `${pct(A[d.key].missRate.p, 1)} of cycles`, 'Empty at the exit', 'Longer fill dwell; vibration']],
    lane: [['Two pills leave together', 'Round pills roll or stagger past the clamp pad', (d) => `${pct(A[d.key].multiRate.p, 1)} of cycles`, 'Exit curtain / load cell', 'Steeper pad hold; lane height below 1.3 × thickness'], ['Queue starves', 'Pills stand on end at the lane entrance', (d) => `${pct(A[d.key].missRate.p, 1)} of cycles`, 'No pill for 10 cycles', 'Floor vibration; longer feed opening'], ['Trapdoor pinch', 'Pill straddles the flap edge', (d) => `${A[d.key].jamsPerDose.toFixed(2)} jams / dose`, 'Flap servo torque', 'Flap hinged at the downhill end']],
    belt: [['Count error → wrong dose', 'Pills overlap: single-height barrier or edge counting reads one', (d) => `${pct(1 - A[d.key].doseRate.p, 0)} of doses not exact`, 'Load cell in the tray; second barrier height', 'Encoder-based length count; upper barrier'], ['Pill damaged at the roller nip', 'Spring-loaded roller squeezes a soft pill', (d) => `${A[d.key].dmgPer100.toFixed(1)} broken / 100`, 'None in-process', 'Lower spring rate; softer roller; larger clearance'], ['No pulse for 6 s', 'Pills bridge under the lip', (d) => `${A[d.key].errors} of ${A[d.key].n} doses stopped`, 'Watchdog', 'Reverse feed belt, shake hopper']],
    vacdisc: [['Flat pill will not seal', 'Rim radius 50 mm vs a 17 mm caplet: chord sag leaks', (d) => { const w = worst(d, (a) => a.doseRate.p, -1); return `worst: ${PRESET[w.k]} ${pct(w.v)} exact`; }, 'Port pressure never drops', 'Flat vacuum plate instead of a disc; smaller ports'], ['Double pick', 'Wedge gap too big for the pill; two pills sealed to one port', (d) => `${pct(A[d.key].multiRate.p, 1)} of cycles`, 'Release barrier; port pressure', 'Singulator gap = 1.25 × thickness + 0.6 mm'], ['Vacuum lost', 'Port clogged with dust or seal leak', (d) => `${A[d.key].errors} of ${A[d.key].n} doses stopped`, 'Pressure signal', 'Purge and brush; filter on the line']],
  };
  $('tblFmea').innerHTML = `<thead><tr><th>Concept</th><th>Failure mode</th><th>Cause (physics)</th><th>Observed on the bench</th><th>Detected by</th><th>Prevented / recovered by</th></tr></thead><tbody>${designs.map((d) => FM[d.key].map((f, i) => `<tr>${i === 0 ? `<td rowspan="3" style="vertical-align:top"><span class="chip"><i style="background:${d.color}"></i>${label(d)}</span></td>` : ''}<td class="wrap"><b>${f[0]}</b></td><td class="wrap">${f[1]}</td><td class="wrap">${f[2](d)}</td><td class="wrap">${f[3]}</td><td class="wrap">${f[4]}</td></tr>`).join('')).join('')}</tbody>`;
