...
reading Rishav's profile. It is optional — the site ships working without it —
...
Nothing visible breaks. The site tries the Worker, and on any failure —
network, HTTP error, empty stream, or a response slower than 18 seconds — it
falls back to the written answers in `src/data/knowledge.ts` and tells the
...
- **`SYSTEM` in `src/index.js`** — the rules. Tone, length, what it must refuse,
  what it must never invent. Edit here to change *how* it answers.
- **`src/data/profile.ts`** in the site — the facts. The corpus is assembled
