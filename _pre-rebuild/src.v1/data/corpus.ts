...
chunks.push({
  ...
  text:
    ...
    `He also ships: his consulting work moved new client enquiries 37% and cut administrative workload 43%, ` +
    `and he has founded or co-led four ventures with three separate awards behind them.`,
});
...
for (const role of ROLES) {
  chunks.push({
    ...
    text:
      ...
      (role.metrics.length
        ? `Measured outcomes: ${role.metrics.map((m) => `${m.value} ${m.label}`).join("; ")}. `
        : "") +
...
chunks.push({
  ...
  tags: [
    "award", "awards", "honor", "honour", "prize", "won", "win", "recognition",
    "grant", "scholarship", "competition", "pitch", "upitch", "kickstart",
  ],
  text: HONORS.map(
